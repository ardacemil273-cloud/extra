...
export const BARBIE_THEMES = [
  {
    id: "malibu-beach",
    name: "Malibu Beach Party",
    emoji: "🏖️",
    desc: "Güneş, kum ve pembe! Malibu tarzını giy!",
  },
  {
    id: "dreamhouse-pajama",
    name: "Dreamhouse Pajama Party",
    emoji: "🛏️",
    desc: "Rahat ve şirin pijama partisi!",
  },
  {
    id: "princess-ball",
    name: "Princess Ball",
    emoji: "👑",
    desc: "Prenses balosu için ışıltılı bir prenses elbisesi!",
  },
  {
    id: "fashion-week-paris",
    name: "Fashion Week Paris",
    emoji: "🗼",
    desc: "Paris moda haftası! Zirvede ol!",
  },
  {
    id: "barbie-ken-date",
    name: "Barbie & Ken Date Night",
    emoji: "💘",
    desc: "Ken ile romantik bir akşam yemeği randevusu!",
  },
];
...
export const BARBIE_WARDROBE: WardrobeItem[] = [
  // Dresses
  {
    id: "dress-pink-tulle",
    category: "dress",
    name: "Pembe Tül Elbise",
    emoji: "👗",
    color: "#ff69b4",
  },
  {
    id: "dress-malibu-swim",
    category: "dress",
    name: "Malibu Mayo",
    emoji: "👙",
    color: "#ffb6c1",
  ...
  {
    id: "dress-princess-gown",
    category: "dress",
    name: "Prenses Balo Elbisesi",
    emoji: "👸",
    color: "#ffc0cb",
  },
  {
...
export class BarbieDressupService {
  ...
  async startGame(
    ...
    const match = await this.prisma.match.create({
      data: {
        ...
        players: { create: players.map((p) => ({ userId: p.userId })) },
      },
    });
    this.roomToMatchId.set(roomId, match.id);

    server.to(`game:${roomId}`).emit("game:started", {
      gameType: "barbie-dreamhouse",
      matchId: match.id,
    });
  ...
  private async startRound(
    ...
    barbie.submitted = [];
    barbie.votes = [];
    barbie.runwayOrder = Object.keys(state.players).sort(
      () => Math.random() - 0.5,
    );
    barbie.runwayPos = -1;
    barbie.roundResults = {};
    ...
    server
      .to(`game:${roomId}`)
      .emit("barbie:round-start", { round, theme, dressTime: DRESS_TIME });
    server.to(`game:${roomId}`).emit("barbie:state-update", barbie);

    const timer = setTimeout(
      () => this.startBoxReveal(roomId, server),
      DRESS_TIME * 1000,
    );
    this.timers.set(`${roomId}:dress`, timer);
  }

  async handlePickItem(
    roomId: string,
    userId: string,
    itemId: string,
    server: Server,
    ...
    const state = await this.gameEngine.getState(roomId);
    if (!state || state.phase !== "DRESSING")
      throw new BadRequestException("Şu an giyinme fazı değil");
    const barbie = (state as any).barbie as BarbieState;
    if (!barbie || barbie.phase !== "DRESSING")
      throw new BadRequestException("Şu an giyinme fazı değil");
    ...
    if (items.length >= 8 && !items.includes(itemId))
      throw new BadRequestException("Maksimum 8 parça");

    const others = items.filter(
      (id) =>
        BARBIE_WARDROBE.find((w) => w.id === id)?.category !== item.category,
    );
    items = [...others, itemId];
  ...
  async handleMakeup(
    ...
    const item = BARBIE_WARDROBE.find(
      (w) => w.id === itemId && w.category === "makeup",
    );
  ...
  private async startBoxReveal(roomId: string, server: Server): Promise<void> {
    ...
    server
      .to(`game:${roomId}`)
      .emit("barbie:box-reveal", { runwayOrder: barbie.runwayOrder });
    server.to(`game:${roomId}`).emit("barbie:state-update", barbie);
  ...
  private showLook(roomId: string, server: Server, pos: number): void {
    ...
    const run = async () => {
      ...
      server.to(`game:${roomId}`).emit("barbie:show-look", {
        ...
        phrase:
          BARBIE_PHRASES[Math.floor(Math.random() * BARBIE_PHRASES.length)],
      });

      const timer = setTimeout(
        () => this.showLook(roomId, server, pos + 1),
        RUNWAY_TIME * 1000,
      );
  ...
  async handleVote(
    ...
    const state = await this.gameEngine.getState(roomId);
    if (!state || state.phase !== "RUNWAY")
      throw new BadRequestException("Şu an podyum fazı değil");
    const barbie = (state as any).barbie as BarbieState;
    if (!barbie || barbie.phase !== "RUNWAY")
      throw new BadRequestException("Şu an podyum fazı değil");

    if (userId === targetId)
      throw new BadRequestException("Kendinize oy veremezsiniz");
    if (stars < 1 || stars > 5)
      throw new BadRequestException("1-5 yıldız arası oy verin");
    if (!state.players[targetId])
      throw new BadRequestException("Hedef bulunamadı");

    const showing = barbie.runwayOrder[barbie.runwayPos];
    if (targetId !== showing)
      throw new BadRequestException("Bu oyuncu şu an podyumda değil");

    const existing = barbie.votes.find(
      (v) => v.voterId === userId && v.targetId === targetId,
    );
  ...
  private async endRound(roomId: string, server: Server): Promise<void> {
    ...
    for (const uid of barbie.runwayOrder) {
      ...
      if (votes.length === 0) scores[uid] = 4;
        ...
        scores[uid] =
          Math.round(
            (votes.reduce((a, v) => a + v.stars, 0) / votes.length) * 10,
          ) / 10;
    }
    ...
    if (barbie.round >= 4) {
      ...
      const timer = setTimeout(
        () => this.startRound(roomId, server, barbie.round + 1),
        6000,
      );
    ...
    if (matchId) {
      await this.prisma.match.update({
        ...
        data: {
          ...
          endedAt: new Date(),
          winnerId,
          winnerTeam: winnerId ? "BARBIE" : "NONE",
