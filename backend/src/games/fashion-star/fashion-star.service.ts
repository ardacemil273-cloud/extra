...
export type ItemCategory =
  "top" | "bottom" | "dress" | "shoes" | "accessory" | "hair";
...
export const THEMES = [
  {
    ...
    name: "İlk Buluşma",
    emoji: "💘",
    desc: "Romantik ve zarif görün, kalpleri kazan!",
  },
  {
    id: "Festival",
    name: "Festival",
    emoji: "🎪",
    desc: "Renkli ve eğlenceli, festival ruhunu yakala!",
  },
  {
    id: "Wedding",
    name: "Düğün",
    emoji: "💍",
    desc: "Işıltılı ve muhteşem, düğün gecesine hazır ol!",
  },
];
...
export class FashionStarService {
  ...
  async startGame(
    ...
    server
      .to(`game:${roomId}`)
      .emit("game:started", { gameType: "fashion-star", matchId: match.id });
  ...
  private async startRound(
    ...
    fashion.submitted = [];
    fashion.votes = [];
    fashion.runwayOrder = Object.keys(state.players).sort(
      () => Math.random() - 0.5,
    );
    fashion.runwayPos = -1;
    ...
    server
      .to(`game:${roomId}`)
      .emit("fashion:round-start", { round, theme, dressTime: DRESS_TIME });
    server.to(`game:${roomId}`).emit("fashion:state-update", fashion);

    const timer = setTimeout(
      () => this.startRunway(roomId, server),
      DRESS_TIME * 1000,
    );
  ...
  async handlePickItem(
    ...
    const fashion = (state as any).fashion as FashionState;
    if (!fashion || fashion.phase !== "DRESSING")
      throw new BadRequestException("Şu an giyinme fazı değil");
    ...
    const others = items.filter(
      (id) => WARDROBE.find((w) => w.id === id)?.category !== item.category,
    );
    items = [...others, itemId];
  ...
  private async startRunway(roomId: string, server: Server): Promise<void> {
    ...
    server
      .to(`game:${roomId}`)
      .emit("fashion:runway-start", { runwayOrder: fashion.runwayOrder });
    server.to(`game:${roomId}`).emit("fashion:state-update", fashion);
  ...
  async handleVote(
    ...
    const fashion = (state as any).fashion as FashionState;
    if (!fashion || fashion.phase !== "RUNWAY")
      throw new BadRequestException("Şu an podyum fazı değil");
    ...
    const existing = fashion.votes.find(
  ...
  private async endRound(roomId: string, server: Server): Promise<void> {
    ...
    if (fashion.round >= 2) {
      ...
      const timer = setTimeout(
        () => this.startRound(roomId, server, fashion.round + 1),
        6000,
      );
    ...
    if (matchId) {
      await this.prisma.match.update({
        ...
        data: {
          ...
          endedAt: new Date(),
          winnerId,
          winnerTeam: winnerId ? "FASHION" : "NONE",
