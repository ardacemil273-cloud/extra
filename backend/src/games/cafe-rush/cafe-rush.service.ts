...
export type RecipeId =
  "strawberry-cake" | "burger" | "salad" | "pancake" | "smoothie";
...
export class CafeRushService {
  ...
  async startGame(
    ...
    server
      .to(`game:${roomId}`)
      .emit("game:started", { gameType: "cafe-rush", matchId: match.id });
  ...
  private async tick(roomId: string, server: Server): Promise<void> {
    ...
    const now = Date.now();
    cafe.remaining = Math.max(
      0,
      Math.round(cafe.duration - (now - cafe.startTime) / 1000),
    );
    ...
    cafe.orders = cafe.orders.filter(
      (o) =>
        o.status !== "EXPIRED" ||
        (o.status === "EXPIRED" && now - o.expiresAt < 2000),
    );

    await this.gameEngine.updateState(roomId, (s) => ({ ...s, cafe }));
    server
      .to(`game:${roomId}`)
      .emit("cafe:tick", { remaining: cafe.remaining, orders: cafe.orders });
    server.to(`game:${roomId}`).emit("cafe:state-update", cafe);
  ...
  async handleAction(
    ...
    const state = await this.gameEngine.getState(roomId);
    if (!state || state.phase !== "SERVING")
      throw new BadRequestException("Oyun aktif değil");
    const cafe = (state as any).cafe as CafeState;
    if (!cafe || cafe.status !== "PLAYING")
      throw new BadRequestException("Oyun aktif değil");

    const order = cafe.orders.find((o) => o.id === orderId);
    if (!order || order.status !== "ACTIVE")
      throw new BadRequestException("Sipariş bulunamadı ya da süresi doldu");

    const recipe = RECIPES[order.recipe];

    if (action === "claim") {
      if (order.assignedTo && order.assignedTo !== userId)
        throw new BadRequestException("Bu sipariş başkasına ait");
      order.assignedTo = userId;
    } else if (action === "advance") {
      if (order.assignedTo !== userId)
        throw new BadRequestException("Bu siparişi siz hazırlamıyorsunuz");
      order.stepIndex++;
  ...
  private async endGame(
    ...
    server.to(`game:${roomId}`).emit("game:over", {
      ...
      narrator: won
        ? "Mutfak harika çalıştı! 15 sipariş tamamlandı! 🍰"
        : "Süre doldu... Siparişler yetişmedi!",
      players: results,
    });
    ...
    if (matchId) {
      await this.prisma.match.update({
        ...
        data: {
          status: "FINISHED",
          endedAt: new Date(),
          winnerTeam: won ? "CAFE" : "NONE",
          duration: Math.floor((Date.now() - state.startedAt) / 1000),
          state: { served: cafe.served, won },
        },
      });
      for (const r of results) {
        await this.prisma.matchPlayer.updateMany({
          ...
          data: {
            role: r.role,
            isWinner: r.isWinner,
            score: cafe.playerCooks[r.userId] || 0,
          },
        });
