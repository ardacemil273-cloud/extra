...
import {
  ...
  GamePlayer,
  GameResult,
} from "../../game-engine/interfaces/game.interfaces";
...
const NARRATOR_LINES = {
  ...
  vampire_turn: [
    "Vampir, bu gece kimin kanını içeceğini seçiyor...",
    "Gece ilerliyor... Vampirin pençeleri uzanıyor.",
  ],
  doctor_turn: [
    "Doktor, bugün kimi koruyacağını belirliyor...",
    "Şifacı elleri, bu gece bir hayatı kurtarabilir.",
  ],
  detective_turn: [
    "Dedektif gözlerini kısıyor, şüphelisini sorguluyor...",
    "Akıl yürütme vakti. Dedektif gerçeği arıyor.",
  ],
  ...
  no_death: [
    "Bir mucize! Bu sabah kimse ölmedi. Ama tehlike hâlâ aramızda.",
    "Güvenli bir gece geçirdik. Ama ne kadar sürer?",
  ],
  player_died: (name: string) => [
    `Sabah olduğunda ${name} artık aramızda yoktu...`,
    `${name} gece vampirin kurbanı oldu. Kolu soğuk, yüzü solgundu...`,
  ],
  discussion: [
    "Konuşma vakti. Kim güvenilir, kim şüpheli? Dikkatli düşünün.",
    "Aramızdaki vampiri bulmanın tek yolu konuşmak. Başlayın.",
  ],
  voting: [
    "Artık karar verme zamanı. Oyunuzu kullanın.",
    "Oylama başlıyor. Her oy belirleyici olabilir.",
  ],
  ...
  vampire_wins: [
    "VAMPIRLER KAZANDI! Köy, karanlığa yenik düştü...",
    "Vampirler tüm kurbanlarını tüketmiş, köyü ele geçirdi!",
  ],
  villagers_win: [
    "KÖYLÜLER KAZANDI! Işık karanlığı yendi!",
    "Vampir avlandı! Köy kurtuldu, zafer köylülerin!",
  ],
...
export class VampireVillageService {
  ...
  async startGame(
    ...
    if (room.players.length < 4)
      throw new BadRequestException("En az 4 oyuncu gerekli");

    // Oda ayarlarını oku (host panelinden kaydedilen)
    const settings = (room.settings as any) || {};
    const gameSettings = {
      vampireCount: Math.min(
        settings.vampireCount || 1,
        Math.floor(room.players.length / 4),
      ),
    ...
    server
      .to(`game:${roomId}`)
      .emit("game:narrator", { message: narratorLine, type: "game_start" });
    server
      .to(`game:${roomId}`)
      .emit("game:started", { gameType: "vampire-village", matchId: match.id });
  ...
  private async processNightRoles(
    roomId: string,
    server: Server,
    state: GameState,
  ): Promise<void> {
  ...
  private async awakeRole(
    ...
    if (socketId) {
      ...
      server.to(socketId).emit("game:your-turn", {
        role,
        targets,
        duration: PHASE_DURATIONS.NIGHT,
      });
    }
  }
  ...
  async handleAction(
    ...
    if (player.actionDone)
      throw new BadRequestException("Bu tur zaten aksiyonunuzu kullandınız");
    ...
    switch (action) {
      case "VAMPIRE_KILL":
        if (player.role !== "VAMPIRE")
          throw new ForbiddenException("Bu aksiyonu kullanamazsınız");
        await this.gameEngine.updateState(roomId, (s) => ({
          ...s,
          nightDeaths: [...s.nightDeaths, targetId],
          players: {
            ...s.players,
            [userId]: { ...s.players[userId], actionDone: true },
          },
        }));
        break;
  ...
  private async resolveNight(roomId: string, server: Server): Promise<void> {
    ...
    server.to(`game:${roomId}`).emit("game:morning", {
      ...
      deathAnnouncement,
      savedByDoctor:
        state.savedByDoctor !== null &&
        state.nightDeaths.includes(state.savedByDoctor),
    });
    ...
    setTimeout(
      () => this.startDiscussion(roomId, server),
      PHASE_DURATIONS.DAY_ANNOUNCEMENT * 1000,
    );
  }
  ...
  private async resolveVoting(roomId: string, server: Server): Promise<void> {
    ...
    if (results.length === 0 || results[0].votes === 0) {
      ...
      setTimeout(
        () => this.startNight(roomId, server),
        PHASE_DURATIONS.VOTE_RESULT * 1000,
      );
      return;
    }
    ...
    if (winner) {
      setTimeout(
        () => this.endGame(roomId, winner, server),
        PHASE_DURATIONS.VOTE_RESULT * 1000,
      );
    } else {
      setTimeout(
  ...
  private assignRoles(
  ...
  ): Record<string, { role: string; team: string }> {
    const count = players.length;
    const shuffled = [...players].sort(() => Math.random() - 0.5);
    const roleMap: Record<string, { role: VampireRole; team: string }> = {};

    const vampireCount = settings
      ? Math.max(1, Math.min(settings.vampireCount, Math.floor(count / 4)))
    ...
    if (
      (settings ? settings.hasDoctor : true) &&
      count >= 5 &&
      idx < shuffled.length
    ) {
    ...
    if (
      (settings ? settings.hasDetective : true) &&
      count >= 6 &&
      idx < shuffled.length
    ) {
      roleMap[shuffled[idx++].userId] = {
        role: "DETECTIVE",
        team: "VILLAGERS",
      };
    }
    if (
      (settings ? settings.hasHunter : false) &&
      count >= 8 &&
      idx < shuffled.length
    ) {
