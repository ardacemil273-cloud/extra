...
export default function JoinRoomPage() {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('şifre') || msg.toLowerCase().includes('password')) {
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
        ...
        <div className="rounded-2xl border border-white/8 bg-white/3 p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400 text-sm">Arkadaşından aldığın 6 haneli kodu gir</p>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-white/5 text-center">
            ...
            <Link href="/room/create" className="text-sm text-purple-400 hover:text-purple-300 font-medium">
              Oda Oluştur →
            </Link>
          </div>
        </div>
