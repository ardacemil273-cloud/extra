...
import { calculateLevelProgress, getRarityColor, getRarityLabel } from '@/lib/utils';
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    loadAchievements();
    if (user?.profile) setForm({ displayName: user.profile.displayName, bio: user.profile.bio || '', avatar: user.profile.avatar || 'default' });
  }, [isAuthenticated]);
      ...
      <main className="relative z-10 max-w-4xl mx-auto px-6 py-8 space-y-6">
            ...
            <div className="flex-1">
              {editing ? (
                <input type="text" value={form.displayName} onChange={(e) => setForm({ ...form, displayName: e.target.value })}
                  className="text-2xl font-black bg-white/5 border border-white/10 rounded-xl px-3 py-2 w-full focus:outline-none focus:border-purple-500 mb-2" maxLength={50} />
              ) : (
                <h2 className="text-2xl font-black">{profile?.displayName || user?.username}</h2>
              )}
              <p className="text-gray-400 text-sm mb-1">@{user?.username}</p>
              {editing ? (
                <textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })}
                  placeholder="Kendin hakkında bir şeyler yaz..."
                  className="w-full mt-2 px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-gray-300 placeholder-gray-600 focus:outline-none focus:border-purple-500 resize-none h-20" maxLength={200} />
              ) : (
          ...
          <div className="rounded-2xl border border-white/8 bg-white/3 p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Kazanma %', value: `${Math.round((stats.winRate || 0) * 100)}%`, icon: '📊' },
              ].map((s) => (
                <div key={s.label} className="rounded-xl border border-white/5 bg-white/2 p-4 text-center">
        ...
        <div className="rounded-2xl border border-white/8 bg-white/3 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">{unlocked.length}/{achievements.length}</span>
          </div>
              ...
              <motion.div key={ach.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold truncate">{ach.name}</span>
                    <span className="text-xs px-1.5 py-0.5 rounded-full border" style={{ color: getRarityColor(ach.rarity), borderColor: getRarityColor(ach.rarity) + '40', backgroundColor: getRarityColor(ach.rarity) + '15' }}>
                      {getRarityLabel(ach.rarity)}
                    </span>
                  </div>
                  <p className="text-xs text-gray-500 truncate">{ach.description}</p>
                </div>
                {ach.unlocked && <span className="text-green-400 text-sm">✓</span>}
                <span className="text-xs text-yellow-500/70">+{ach.xpReward}xp</span>
              </motion.div>
            ))}
          </div>
        </div>
